...
namespace valcrypt {
    ...
    template<typename T>
    T CEncryptedValue<T>::Cipher(T value)  {
        volatile T result = value;
        result = valcrypt_xor<T>(result, m_Key);
        return result;
    }

    template uint8_t CEncryptedValue<uint8_t>::Cipher(uint8_t);
    template uint16_t CEncryptedValue<uint16_t>::Cipher(uint16_t);
    template uint32_t CEncryptedValue<uint32_t>::Cipher(uint32_t);
    template uint64_t CEncryptedValue<uint64_t>::Cipher(uint64_t);
    template int8_t CEncryptedValue<int8_t>::Cipher(int8_t);
    template int16_t CEncryptedValue<int16_t>::Cipher(int16_t);
    template int32_t CEncryptedValue<int32_t>::Cipher(int32_t);
    template int64_t CEncryptedValue<int64_t>::Cipher(int64_t);
