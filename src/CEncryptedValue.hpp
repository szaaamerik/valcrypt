...
namespace valcrypt {
    ...
    public:
        ...
        VALCRYPT_FORCEINLINE T GetDecrypted() {
        ...
        CEncryptedValue operator+(const CEncryptedValue& other) const {
            return CEncryptedValue(Cipher(m_EncryptedValue) + other.Cipher(other.m_EncryptedValue));
        }
        ...
        CEncryptedValue operator+(const T other) const {
            return CEncryptedValue(Cipher(m_EncryptedValue) + other);
        }
        ...
        bool operator==(const CEncryptedValue& other) const {
            return Cipher(m_EncryptedValue) == other.Cipher(other.m_EncryptedValue);
        }
    ...
    private:
        #ifdef _MSC_VER
            VALCRYPT_FORCEINLINE T Cipher(T value);
        #else
            T Cipher(T value);
        #endif
